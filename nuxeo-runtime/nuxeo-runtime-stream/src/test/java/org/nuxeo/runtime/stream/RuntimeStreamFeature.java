 ...
 * <p/>
 * To run your unit tests on a Chronicle or Kafka you need to declare {@code nuxeo.test.stream.type} to either
 ...
 *     <logConfig name="MY_LOG_CONFIG_NAME" type="${nuxeo.test.stream.type}" />
 *   </extension>
 * }
 * </pre>
 *
...
public class RuntimeStreamFeature implements RunnerFeature {
    ...
    public static final String STREAM_PROPERTY = "nuxeo.test.stream.type";
    ...
    protected void initChronicle(RuntimeHarness harness) {
        log.info("Deploying Chronicle for stream services");
        // nothing to do
    }

    protected void initKafka(RuntimeHarness harness) throws IOException {
        log.info("Deploying Kafka for stream services");
        ...
        URL contribURL = bundle.getEntry("OSGI-INF/test-stream-kafka-contrib.xml");
