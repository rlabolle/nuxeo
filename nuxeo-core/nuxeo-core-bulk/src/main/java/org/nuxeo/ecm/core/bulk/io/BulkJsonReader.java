...
import static org.nuxeo.ecm.core.bulk.io.BulkConstants.BULK_CREATION;
...
public class BulkJsonReader extends EntityJsonReader<BulkStatus> {
    ...
    public BulkStatus readEntity(JsonNode jn) throws IOException {
        ...
        String creation = getStringField(jn, BULK_CREATION);

        if (isNotEmpty(creation)) {
            status.setCreationInstant(Instant.parse(creation));
        }
