 ...
 * <p/>
 * To run your unit tests on a {@link StreamWorkManager} you need to declare {@code nuxeo.test.workmanager.type=stream}
...
public class WorkManagerFeature implements RunnerFeature {
    ...
    public static final String WORK_MANAGER_PROPERTY = "nuxeo.test.workmanager.type";
    ...
    // TODO copied from StorageConfiguration
    ...
    protected void initDefaultImplementation(RuntimeHarness harness) throws IOException {
        log.info("Deploying default work manager implementation");
        // deploy component
        deployComponent(harness, "OSGI-INF/test-default-workmanager-config.xml");
    }

    protected void initStreamImplementation(RuntimeHarness harness) throws IOException {
        log.info("Deploying stream work manager implementation");
        ...
        // deploy component
        deployComponent(harness, "OSGI-INF/test-stream-workmanager-config.xml");
    }

    protected void deployComponent(RuntimeHarness harness, String entryPath) throws IOException {
        Bundle bundle = harness.getOSGiAdapter().getRegistry().getBundle(BUNDLE_TEST_NAME);
        URL contribURL = bundle.getEntry(entryPath);
